...
const api: InternshipOsApi = {
  ...
  resume: {
    ...
    createJobDraft: (name: string, replace = false) => ipcRenderer.invoke('resume:create-job-draft', name, replace),
    setJobDraftActive: (active: boolean) => ipcRenderer.invoke('resume:set-job-draft-active', active),
    discardJobDraft: () => ipcRenderer.invoke('resume:discard-job-draft'),
